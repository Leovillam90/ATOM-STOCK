...
export default function ReportesPage() {
  ...
  // Cálculo de Costos (Unit Economics) y Unidades Entregadas
    ...
    if (Array.isArray(v.items)) {
      v.items.forEach((it: any) => {
        ...
        if (prod) {
          ...
          costoTotalAcumulado += (cImp + cFul) * cant;
        }
      });
    }
  });

  const gananciaEstimada = totalVentasEntregadas > 0 
    ? (costoTotalAcumulado > 0 ? totalVentasEntregadas - costoTotalAcumulado : totalVentasEntregadas * 0.3)
    : 0;

  const porcentajeMargenNeto = totalVentasEntregadas > 0 
    ? Math.round((gananciaEstimada / totalVentasEntregadas) * 100) 
    : 0;

  // Contador de Devoluciones del Periodo
  const totalDevoluciones = ventasFiltradas.filter(v => {
    const est = String(v.estado || '').toUpperCase();
    return est.includes('DEVOLUCION') || est.includes('DEVOLUCIÓN') || est.includes('ANULADA');
  }).length;

  // 3. AGRUPACIÓN DINÁMICA DE VENTAS (POR SEDE, CANAL O VENDEDOR)
  const obtenerAgrupacion = () => {
    const mapa: { [key: string]: number } = {};

    ventasEntregadasFiltradas.forEach(v => {
      let clave = 'Sede Principal';
      if (agruparPor === 'SEDES') clave = v.nombre_bodega || v.id_bodega_despacho || 'Sede Principal';
      if (agruparPor === 'VENDEDORES') clave = v.vendedor_nombre || 'Vendedor POS';
      if (agruparPor === 'CANALES') clave = v.canal_origen || 'Ecommerce Dropi';
  ...
  // 4. RANKING TOP PRODUCTOS MÁS VENDIDOS (SOLO DE ÓRDENES EN ESTADO 'ENTREGADO')
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[11px] font-satoshi-black text-[#0DE8C0] uppercase tracking-wider">
              Consolidado Ecommerce Dropi & POS
            </span>
          </div>
          ...
          <p className="text-xs text-[#A0AEC0] mt-1 font-satoshi-regular max-w-xl">
            Monitoreo en tiempo real del flujo de caja, rendimiento por sedes y ventas entregadas.
          </p>
        </div>
      </div>
        ...
        <div className="lg:col-span-3 bg-[#253443] border border-slate-700/50 rounded-2xl p-6 shadow-xl flex flex-col justify-between h-44">
          ...
          <div className="my-1">
            <div className="text-3xl font-black text-[#C81FDA] font-satoshi-black tracking-tight">
              {formatoCOP(gananciaEstimada)}
            </div>
          </div>

          <p className="text-xs text-[#A0AEC0] font-satoshi-regular">
            Utilidad calculada sobre Unit Economics
          </p>
        </div>

        {/* UNIDADES Y DEVOLUCIONES (3 COLS) */}
        <div className="lg:col-span-3 bg-[#253443] border border-slate-700/50 rounded-2xl p-5 shadow-xl flex flex-col justify-between h-44">
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-black text-white font-satoshi-black">{totalUnidadesVendidas} unds</span>
            </div>
            ...
            <div className="flex items-center justify-between border-t border-slate-700/60 pt-2">
              ...
              <span className="text-sm font-black text-white font-satoshi-black">{ventasEntregadasFiltradas.length} ops</span>
            </div>
          </div>

          <div className="bg-amber-950/40 border border-amber-500/40 rounded-xl p-2.5 flex items-center justify-between text-xs">
            <div className="flex items-center gap-2 text-amber-400 font-satoshi-black">
              <svg className="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
              </svg>
              <span>Devoluciones:</span>
            </div>
            <span className="font-satoshi-black text-amber-300">{totalDevoluciones} {totalDevoluciones === 1 ? 'Devolución' : 'Devoluciones'}</span>
          </div>
        </div>
          ...
          <div className="border-b border-slate-700/60 pb-3 flex justify-between items-center">
            <div>
              <h2 className="text-base font-satoshi-black text-white uppercase tracking-wider">
                Distribución por {agruparPor === 'SEDES' ? 'Sedes' : agruparPor === 'CANALES' ? 'Canales (Ecommerce Dropi / POS)' : 'Vendedor'}
              </h2>
              <p className="text-xs text-[#A0AEC0] font-satoshi-regular">
