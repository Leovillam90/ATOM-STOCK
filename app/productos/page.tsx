...
export default function ProductosPage() {
  ...
  // 4. Filtros de Tabla y Píldoras Dinámicas
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const nuevoCambioAuditoria = editingSku ? {
        ...
        usuario_nombre: userAuth?.nombre || 'Usuario LOBO',
  ...
  const handleDescargarPlantillaProductos = () => {
    ...
    link.href = url;
    link.setAttribute('download', 'Plantilla_Multibodega_Productos_LOBO_STOCK.csv');
  ...
  return (
    <div className="min-h-screen bg-[#F4F5F7] text-gray-800 p-6 md:p-10 font-sans relative pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4 border-b border-gray-200 pb-6">
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <button
            ...
            className="bg-[#FFD800] hover:bg-[#FDCB13] text-[#222222] font-satoshi-black px-6 py-3 rounded-xl text-xs uppercase tracking-wider transition-all duration-300 shadow-sm flex items-center gap-2 shrink-0 font-bold"
          >
      ...
      {/* METRICAS SUPERIORES CON ÍCONOS 2D */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm flex flex-col justify-between min-h-[9.5rem] space-y-3">
          <div className="flex justify-between items-start">
            ...
            <span className="bg-[#222222] text-[#FFD800] text-[10px] font-satoshi-black px-2.5 py-0.5 rounded-full uppercase font-bold">
              {productos.length} {productos.length === 1 ? 'Producto' : 'Productos'}
            </span>
          </div>

          <div>
            <div className="text-3xl font-black text-gray-900 font-satoshi-black leading-tight tracking-tight">
              {formatoCOP(kpis.valorTotalInventario)}
            </div>
          </div>

          <p className="text-xs text-gray-500 font-satoshi-regular truncate">
            Valoración final con impuestos incluidos
          </p>
        </div>

        <div className={`bg-white border rounded-2xl p-5 shadow-sm flex flex-col justify-between min-h-[9.5rem] space-y-3 transition-colors ${
          kpis.totalBajoStockCount > 0 ? 'border-amber-200' : 'border-gray-200'
        }`}>
          <div className="flex justify-between items-start">
            <span className={`text-[11px] font-satoshi-black uppercase tracking-wider font-bold ${
              kpis.totalBajoStockCount > 0 ? 'text-amber-800' : 'text-gray-700'
            }`}>
              REQUIEREN REPOSICIÓN
            </span>
            <div className={`w-8 h-8 rounded-xl flex items-center justify-center ${
              kpis.totalBajoStockCount > 0 ? 'bg-amber-100 text-amber-800' : 'bg-gray-100 text-gray-500'
            }`}>
          ...
          <div>
            <div className={`text-4xl font-black font-satoshi-black leading-tight ${
              kpis.totalBajoStockCount > 0 ? 'text-amber-900' : 'text-gray-900'
            }`}>
              {kpis.totalBajoStockCount} <span className="text-sm font-satoshi-regular text-gray-500">SKUs</span>
            </div>
          </div>
        ...
        <div className={`bg-white border rounded-2xl p-5 shadow-sm flex flex-col justify-between min-h-[9.5rem] space-y-3 transition-colors ${
          kpis.totalInactivos120Count > 0 ? 'border-red-200' : 'border-gray-200'
        }`}>
          <div className="flex justify-between items-start">
            <span className={`text-[11px] font-satoshi-black uppercase tracking-wider font-bold ${
              kpis.totalInactivos120Count > 0 ? 'text-red-800' : 'text-gray-700'
            }`}>
              ESTANCADOS (120+ DÍAS)
            </span>
            <div className={`w-8 h-8 rounded-xl flex items-center justify-center ${
              kpis.totalInactivos120Count > 0 ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-500'
            }`}>
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div>
            <div className={`text-4xl font-black font-satoshi-black leading-tight ${
              kpis.totalInactivos120Count > 0 ? 'text-red-900' : 'text-gray-900'
            }`}>
              {kpis.totalInactivos120Count} <span className="text-sm font-satoshi-regular text-gray-500">SKUs</span>
            </div>
          </div>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-4 mb-8 space-y-4 shadow-sm">
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3 w-full md:w-auto flex-1">
            ...
            <div className="flex items-center gap-2 bg-gray-50 border border-gray-300 rounded-xl px-3 py-1.5">
              <span className="text-[10px] font-satoshi-black text-gray-600 uppercase font-bold">Sede:</span>
              <select
              ...
              >
                <option value="TODAS" className="bg-white text-gray-900">Todas las Sedes</option>
          ...
          <div className="bg-gray-100 p-1 rounded-xl flex items-center gap-1 shrink-0 border border-gray-200">
            <button
              type="button"
        ...
        <div className="flex flex-wrap items-center gap-2 pt-3 border-t border-gray-100">
          <span className="text-[10px] font-satoshi-black text-gray-500 uppercase pr-2 font-bold">Filtros Inteligentes:</span>
          ...
          >
            <span className="flex items-center gap-1.5">
              <span>Todos</span>
              <span className="bg-black/10 px-1.5 py-0.5 rounded-md text-[10px]">{countsFiltros.todos}</span>
            </span>
          </button>
          ...
          >
            <span className="flex items-center gap-1.5">
              ...
              <span className="bg-black/10 px-1.5 py-0.5 rounded-md text-[10px]">{countsFiltros.bajo}</span>
            </span>
          </button>
          ...
          >
            <span className="flex items-center gap-1.5">
              ...
              <span className="bg-black/10 px-1.5 py-0.5 rounded-md text-[10px]">{countsFiltros.inter}</span>
            </span>
          </button>
          ...
          >
            <span className="flex items-center gap-1.5">
              ...
              <span className="bg-black/10 px-1.5 py-0.5 rounded-md text-[10px]">{countsFiltros.ina60}</span>
            </span>
          </button>
          ...
          >
            <span className="flex items-center gap-1.5">
              ...
              <span className="bg-black/10 px-1.5 py-0.5 rounded-md text-[10px]">{countsFiltros.ina90}</span>
            </span>
          </button>
          ...
          >
            <span className="flex items-center gap-1.5">
              ...
              <span className="bg-black/10 px-1.5 py-0.5 rounded-md text-[10px]">{countsFiltros.ina120}</span>
            </span>
          </button>
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 text-[11px] font-satoshi-black text-gray-600 uppercase tracking-wider border-b border-gray-200">
                <th className="p-4 rounded-tl-2xl">Producto / SKU</th>
                  ...
                  >
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="font-satoshi-black text-gray-900 uppercase text-sm font-bold">{p.nombre}</div>
                          <div className="font-mono text-[10px] text-gray-500 font-bold">SKU: {p.sku}</div>
                        </div>
                      </div>
                    </td>

                    <td className="p-4">
                      <span className="text-[10px] font-satoshi-black uppercase bg-gray-100 text-gray-700 px-2 py-0.5 rounded border border-gray-200 font-bold">
                    ...
                    <td className="p-4 text-center font-satoshi-black">
                      <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold ${
                        stockTot <= 5 
                          ...
                          : (stockTot <= 20 ? 'bg-amber-100 text-amber-800 border border-amber-200' : 'bg-emerald-100 text-emerald-800 border border-emerald-200')
                      }`}>
                        {stockTot} unds
                      </span>
                    </td>

                    <td className="p-4 text-right font-satoshi-black text-gray-900 font-bold">
          ...
          {productosFiltrados.map((p, idx) => {
            ...
            return (
              <div
                key={p.sku || idx}
                onClick={() => handleOpenEdit(p)}
                className="group relative bg-white border border-gray-200 hover:border-gray-300 rounded-2xl p-5 shadow-sm flex flex-col justify-between transition-all duration-300 cursor-pointer"
              >
                <div>
                  <div className="flex justify-between items-start mb-3">
                    <span className="font-mono text-[11px] font-satoshi-black text-gray-800 bg-gray-100 px-2.5 py-0.5 rounded border border-gray-200 font-bold">
                      SKU: {p.sku}
                    </span>

                    <span className={`text-[10px] font-satoshi-black px-2.5 py-0.5 rounded-full font-bold ${
                      stockTot <= 5 
                        ? 'bg-red-100 text-red-800 border border-red-200' 
                        : (stockTot <= 20 ? 'bg-amber-100 text-amber-800 border border-amber-200' : 'bg-emerald-100 text-emerald-800 border border-emerald-200')
                    }`}>
                  ...
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div className="truncate">
                      <h3 className="font-satoshi-black text-sm text-gray-900 uppercase truncate font-bold">
                        {p.nombre}
                      </h3>
                      <p className="text-[10px] text-gray-500 font-satoshi-black uppercase">
                  ...
                  <div className="bg-gray-50 p-3 rounded-xl border border-gray-200 space-y-2 text-xs">
                    <div className="flex justify-between items-center border-b border-gray-200 pb-1.5">
                      <span className="text-[10px] text-gray-500 uppercase font-satoshi-black font-bold">Precio Final Venta:</span>
                      <span className="font-satoshi-black text-gray-900 text-sm font-bold">{formatoCOP(precioFinal)}</span>
                    </div>

                    <div className="grid grid-cols-2 gap-2 text-[10px] text-gray-600">
                      <div>Base Gravable: <span className="font-mono text-gray-900 block">{formatoCOP(base)}</span></div>
                      <div>IVA Discriminado ({tarifaIva}%): <span className="font-mono text-gray-900 font-bold block">{formatoCOP(iva)}</span></div>
                    </div>
                  </div>
                </div>
              </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white border border-gray-200 rounded-2xl p-6 w-full max-w-lg shadow-2xl font-sans max-h-[90vh] overflow-y-auto text-gray-800">
            ...
            {editingSku && (
              <div className="bg-gray-100 p-1 rounded-xl flex items-center gap-1 border border-gray-200 mb-4">
                <button
                  type="button"
            ...
            {activeModalTab === 'DATOS' && (
              <form onSubmit={handleSave} className="space-y-4">
                ...
                <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 space-y-3">
                  <label className="text-xs font-satoshi-black text-gray-900 uppercase flex items-center gap-1.5 font-bold">
                    <svg className="w-4 h-4 text-gray-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                    </svg>
                  ...
                  <div className="flex items-center gap-4">
                    <div className="w-16 h-16 rounded-xl bg-white border border-gray-200 overflow-hidden flex items-center justify-center shrink-0">
                      {imagenUrl ? (
                    ...
                    <div className="flex-1 space-y-2">
                      ...
                      <button
                        ...
                        className="w-full bg-white hover:bg-gray-100 text-gray-800 border border-gray-300 font-satoshi-black py-2 rounded-lg text-xs uppercase transition flex items-center justify-center gap-1.5 font-bold shadow-xs"
                      >
                      ...
                      <input 
                        type="text"
                        className="w-full bg-white border border-gray-300 focus:border-[#FFD800] focus:ring-2 focus:ring-[#FFD800]/20 rounded-lg p-2 text-[11px] text-gray-900 font-satoshi-regular focus:outline-none transition-all"
                ...
                <div>
                  <label className="block text-xs font-satoshi-black text-gray-700 uppercase tracking-wider mb-1.5">Código SKU *</label>
                  <input 
                    type="text"
                    className="w-full bg-gray-50 border border-gray-300 focus:border-[#FFD800] focus:ring-2 focus:ring-[#FFD800]/20 rounded-xl p-3 text-xs text-gray-900 font-mono focus:outline-none disabled:opacity-50 transition-all"
                ...
                <div>
                  <label className="block text-xs font-satoshi-black text-gray-700 uppercase tracking-wider mb-1.5">Nombre del Producto *</label>
                  <input 
                    type="text"
                    className="w-full bg-gray-50 border border-gray-300 focus:border-[#FFD800] focus:ring-2 focus:ring-[#FFD800]/20 rounded-xl p-3 text-xs text-gray-900 focus:outline-none font-satoshi-regular transition-all"
                ...
                <div>
                  <label className="block text-xs font-satoshi-black text-gray-700 uppercase tracking-wider mb-1.5">Categoría *</label>
                  <select
                    className="w-full bg-gray-50 border border-gray-300 focus:border-[#FFD800] focus:ring-2 focus:ring-[#FFD800]/20 rounded-xl p-3 text-xs text-gray-900 font-satoshi-black focus:outline-none cursor-pointer transition-all"
                    value={categoria}
                      ...
                      <div key={suc.id_sucursal || i} className="bg-white border border-gray-200 p-2.5 rounded-lg flex items-center justify-between gap-3 text-xs">
                        ...
                        <div className="flex items-center gap-1.5 shrink-0">
                          <input 
                            type="number"
                            min="0"
                            className="w-24 bg-gray-50 border border-gray-300 focus:border-[#FFD800] focus:ring-2 focus:ring-[#FFD800]/20 rounded-lg p-1.5 text-center font-mono text-gray-900 text-xs [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none transition-all"
                          ...
                          />
                          <span className="text-[10px] text-gray-500">unds</span>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div className="bg-gray-50 border border-gray-200 p-4 rounded-xl space-y-3">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <div>
                      ...
                      <input 
                        type="number"
                        className="w-full bg-white border border-gray-300 focus:border-[#FFD800] focus:ring-2 focus:ring-[#FFD800]/20 rounded-lg p-2.5 text-xs text-gray-900 font-mono focus:outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none transition-all"
                        placeholder="85000"
                        value={pmayor}
                    ...
                    <div>
                      <label className="block text-[10px] font-satoshi-black text-gray-700 mb-1 uppercase font-bold">Tienda POS ($)</label>
                      <input 
                        type="number"
                        className="w-full bg-white border border-gray-300 focus:border-[#FFD800] focus:ring-2 focus:ring-[#FFD800]/20 rounded-lg p-2.5 text-xs text-gray-900 font-mono focus:outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none transition-all"
                        placeholder="129000"
                    ...
                    <div>
                      <label className="block text-[10px] font-satoshi-black text-gray-700 mb-1 uppercase font-bold">E-Commerce ($)</label>
                      <input 
                        type="number"
                        className="w-full bg-white border border-gray-300 focus:border-[#FFD800] focus:ring-2 focus:ring-[#FFD800]/20 rounded-lg p-2.5 text-xs text-gray-900 font-mono focus:outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none transition-all"
                        placeholder="119000"
                ...
                <div className="bg-gray-50 border border-gray-200 p-4 rounded-xl space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-[10px] font-satoshi-black text-gray-600 mb-1 uppercase font-bold">
                ...
                <div className="bg-gray-50 border border-gray-200 p-4 rounded-xl space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2 cursor-pointer select-none">
                      ...
                      />
                      <label htmlFor="checkAplicaIva" className="text-xs font-satoshi-black text-gray-800 cursor-pointer font-bold">
                        ¿Precio Incluye IVA?
                      </label>
                    </div>
                  </div>
                  ...
                  {aplicaIva && (
                    <div className="space-y-3 pt-2">
                      <div>
                        ...
                        <select
                          className="w-full bg-white border border-gray-300 focus:border-[#FFD800] focus:ring-2 focus:ring-[#FFD800]/20 rounded-lg p-2.5 text-xs text-gray-900 font-satoshi-black focus:outline-none transition-all"
                          value={tarifaIva}
                ...
                {editingSku && (
                  <div className="bg-amber-50 border border-amber-200 p-4 rounded-xl space-y-2">
                    <label className="block text-xs font-satoshi-black text-amber-900 uppercase flex items-center gap-1.5 font-bold">
                    ...
                    <textarea
                      rows={2}
                      className="w-full bg-white border border-amber-300 focus:border-[#FFD800] rounded-xl p-2.5 text-xs text-gray-900 placeholder-gray-400 focus:outline-none font-satoshi-regular transition-all"
              ...
              <div className="space-y-3 py-2">
                ...
                <div className="max-h-80 overflow-y-auto space-y-2.5 pr-1">
                  {historialCambios.map((cambio: any, idx: number) => (
                    <div key={idx} className="bg-gray-50 border border-gray-200 rounded-xl p-3.5 space-y-1 text-xs">
                      <div className="flex justify-between items-center">
                        <span className="font-satoshi-black text-gray-900 flex items-center gap-1 font-bold">
                          <svg className="w-3.5 h-3.5 text-gray-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      <p className="text-gray-700 font-satoshi-regular bg-white p-2 rounded-lg border border-gray-200 mt-1">
                ...
                <div className="pt-3 border-t border-gray-100">
                  <button 
                    type="button" 
                    onClick={() => setActiveModalTab('DATOS')}
                    className="w-full bg-gray-100 hover:bg-gray-200 text-gray-800 font-satoshi-black py-2.5 rounded-xl text-xs uppercase transition-colors"
                  >
        ...
        <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white border border-gray-200 rounded-2xl p-6 w-full max-w-md shadow-2xl font-sans text-gray-800">
            <div className="flex justify-between items-center mb-6 border-b border-gray-100 pb-3">
              <h3 className="text-lg font-satoshi-black text-gray-900 uppercase font-bold">Carga Masiva de Productos</h3>
            ...
            <form onSubmit={handleProcesarCargaMasiva} className="space-y-4">
              <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 text-xs text-gray-700 space-y-3">
                ...
                <p className="text-[11px] text-gray-500 leading-relaxed">
                  Soporta Sede, Precios, Costos e IVA. Si un SKU ya existe, actualizará automáticamente sus valores e inventario.
                </p>
                <button 
                  type="button"
